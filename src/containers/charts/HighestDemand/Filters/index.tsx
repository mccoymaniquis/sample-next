...
import { getHighestDemandFilters, setHighestDemandBody, setHighestDemandFilters, setLastTouchedFilter } from "@/reducers/Charts";
...
function Filters() {
  ...
  const methods = useForm({
    mode: "onChange",
    defaultValues: {
      demandLevel: _.defaultTo(filters?.demandLevel, DEFAULT_VALUES.demandLevel),
      month: _.defaultTo(filters?.month, DEFAULT_VALUES.month),
      year: _.defaultTo(filters?.year, DEFAULT_VALUES.year),
    },
  });
  ...
  const handleSetTouchedFilter = <T extends ChartsFilterProps<OptionTypeProps>>(data: T) => {
    ...
    if (isMonthFiltered || isYearFiltered) {
      dispatch(setLastTouchedFilter("datePeriod"));
    }

    if (isDemandLevelFiltered) {
      dispatch(setLastTouchedFilter("demandLevel"));
    }
  };
