...
import { useDownloadReports } from "@/services/mutations/reports";
...
function DownloadReports(): ReactElement {
  ...
  const open = useSelector((state: RootState) => state.modal.showReportDownloadModal);
  ...
  const params = {
    ...
    frequency: frequency?.id === "" ? null : frequency?.id,
  ...
  const handleClose = () => {
    dispatch(setModalVisibility({ key: "showReportDownloadModal", value: false }));
  };
  ...
  const onSubmit = async () => {
    try {
      ...
      const blob = {
        blob: response,
        fileName: `Report_${moment(dateToday).format("YYYY-MM-DD")}`,
      };
      downloadBlob(blob);
