...
import { useUploadSupply } from "@/services/mutations/supply";
...
function UploadSupply(): ReactElement {
  const dispatch = useDispatch();
  const open = useSelector((state: RootState) => state.modal.showUploadSupplyModal);
  const [hasMounted, setHasMounted] = useState(false);
  const uploadSupply = useUploadSupply();
  ...
  const handleClose = () => {
    dispatch(setModalVisibility({ key: "showUploadSupplyModal", value: false }));
  };
  ...
  const onSubmit = async (data: UploadFormValues) => {
    try {
      ...
      const result = await uploadSupply.mutateAsync(formData);
      ...
      <FormProvider {...methods}>
        <Modal open={open} onClose={handleClose} width={600} sx={{ px: 2, pt: 2 }}>
          <Box display="flex" justifyContent="space-between" alignItems="center">
            <Typography variant="h6">Upload Supply</Typography>
