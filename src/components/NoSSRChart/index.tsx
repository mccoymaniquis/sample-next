...
import type { ChartProps, CustomYAxisLabelProps } from "@/types/charts";
...
function CombinedChart(props: ChartProps): ReactElement {
  ...
  const demandWonMax: number = _.maxBy(data ?? [], "demandWon")?.demandWon ?? 0;
  const demandWonMin: number = _.minBy(data ?? [], "demandWon")?.demandWon ?? 0;
  const benchWonMax: number = _.maxBy(data ?? [], "benchAfterWon")?.demandWon ?? 0;
  const benchWonMin: number = _.minBy(data ?? [], "benchAfterWon")?.demandWon ?? 0;
        ...
        {hasData
          ? (
              <ResponsiveContainer>
                <ComposedChart
                  data={mappedData}
                  margin={{ top: 20, bottom: 90, right: 30 }}
                >
                  ...
                  <YAxis
                    domain={[demandWonMin, demandWonMax]}
                    yAxisId="left"
                    tick={{
                  ...
                  />
                  <YAxis
                    domain={[benchWonMin, benchWonMax]}
                    yAxisId="right"
                    tick={{
                      fill: theme.palette.text.primary,
                      fontSize: 12,
                  ...
                  <Bar yAxisId="left" dataKey="demandWon" name="Demand Count" fill="#407CF1" />
                  <Bar
                    yAxisId="right"
